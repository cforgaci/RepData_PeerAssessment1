...
As shown in the graph above, the mean total number of steps taken per day is `r mean_total_steps_per_day` and the median is `r median_total_steps_per_day`.
...
As shown in the time series plot above, the activity pattern spikes at a maximum of `r max_interval$mean` steps in the `r max_interval$interval` interval, that is, at 8:35 in the morning.
...
```
From a total of `r length(activity_data$steps)` observations, values are missing for `r nas`, that is `r nas_pct` percent of the total number of observations. Missing values will be filled with the mean of all values from that interval across all days, as follows.
